"""
Tellus globe — 17fps with @micropython.viper upscale
"""
...
MISO=Pin(12,Pin.IN)
spi=SPI(1,baudrate=62_500_000,polarity=0,phase=0,sck=Pin(10),mosi=Pin(11),miso=MISO)
...
cmd(0x11);time.sleep_ms(120);cmd(0x29);time.sleep_ms(100)
...
CONTS=[(0.18,0.28,0.12,0.18,0.3),(0.15,0.35,0.08,0.12,-0.2),(0.22,0.0,0.06,0.20,0.1),(0.25,-0.08,0.04,0.12,-0.1),(0.42,0.30,0.08,0.12,0),(0.45,0.25,0.06,0.08,0.4),(0.40,0.22,0.05,0.06,-0.2),(0.45,0.0,0.08,0.22,0),(0.48,-0.05,0.06,0.15,-0.1),(0.60,0.30,0.18,0.12,0),(0.70,0.25,0.10,0.10,-0.2),(0.65,0.12,0.08,0.10,0.2),(0.70,0.08,0.06,0.08,-0.3),(0.58,0.08,0.04,0.08,0),(0.72,-0.18,0.05,0.06,0),(0.30,-0.38,0.30,0.06,0),(0.28,0.38,0.03,0.04,0),(0.75,0.18,0.02,0.06,0.5),(0.72,0.0,0.03,0.04,0),(0.50,0.15,0.04,0.06,0.3),(0.20,0.08,0.02,0.06,0.4),(0.38,0.33,0.02,0.04,0)]
for ty in range(TX_H):
    lat=(ty-TX_H/2)/(TX_H/2)
    for tx in range(TX_W):
        lon=tx/TX_W;v=0
        for clon,clat,crx,cry,crot in CONTS:
...
for s in range(256):f=s/255;LAND[s]=rgb565(int(60*f),int(130*f),int(45*f));OCEAN[s]=rgb565(int(10*f),int(40*f),int(120*f))
gc.collect()

# Render frames
N=8;FS=FW*FH*2;FRAMES=[None]*N
for fi in range(N):
    rot=fi*TX_W//N
    ...
    for y in range(FH):
        ...
        for x in range(60-hw,61+hw):
            dx=x-60;dz=math.sqrt(3600-dx*dx-dy2)
    ...
    FRAMES[fi]=fb_data;gc.collect()

# Viper upscale
@micropython.viper
def upscale(dst:ptr8,src:ptr8,fw:int,w:int):
    for sy in range(fw):
...
frame=0
while True:
    upscale(buf,FRAMES[frame%N],FW,W)
